...
<% _.forEach(data, function(name, index) { %>
  <% if (_.isArray(name)) { %>
    extern thread_local const AtomicString& k<%= _.camelCase(name[0]) %>;
  <% } else if (_.isObject(name)) { %>
    extern thread_local const AtomicString& k<%= _.camelCase(name.name) %>;
  <% } else { %>
  extern thread_local const AtomicString& k<%= _.camelCase(name) %>;
  <% } %>
<% }) %>
...
<% if (deps && deps.html_attribute_names) { %>
  ...
  <% _.forEach(deps.html_attribute_names.data, function(name, index) { %>
    extern thread_local const AtomicString& k<%= upperCamelCase(_.camelCase(name)) %>Attr;
  <% }) %>
<% } %>
