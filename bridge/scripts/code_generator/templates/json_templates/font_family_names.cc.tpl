...
<% _.forEach(data, function(name, index) { %>
  <% if (_.isArray(name)) { %>
thread_local const AtomicString& k<%= _.camelCase(name[0]) %> = reinterpret_cast<AtomicString*>(&names_storage)[<%= index %>];
  <% } else if (_.isObject(name)) { %>
thread_local const AtomicString& k<%= _.camelCase(name.name) %> = reinterpret_cast<AtomicString*>(&names_storage)[<%= index %>];
  <% } else { %>
thread_local const AtomicString& k<%= _.camelCase(name) %> = reinterpret_cast<AtomicString*>(&names_storage)[<%= index %>];<% } %>
<% }) %>
