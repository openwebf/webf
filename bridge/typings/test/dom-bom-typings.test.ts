...
import { console, fetch, Response, Request, location, history, navigator, asyncStorage, matchMedia, XMLHttpRequest, WebSocket, ResizeObserver} from '../index';
...
function testDocumentAPI() {
  ...
  // Query methods
  const element = doc.querySelector('.class');
  const elements = doc.querySelectorAll('div');
  const byId = doc.getElementById('id');
  const byClass = doc.getElementsByClassName('class');
  const byTag = doc.getElementsByTagName('div');
