...
v8::Local<v8::Value> ScriptValue::V8Value() const {
  ...
  auto scriptState = V8ScriptState::From(isolate_, isolate_->GetCurrentContext());
  return v8_reference_.Get(scriptState->GetIsolate());
}

v8::Local<v8::Value> ScriptValue::V8ValueFor(V8ScriptState* target_script_state) const {
  if (IsEmpty())
    return v8::Local<v8::Value>();
