...
template <typename T>
Persistent<T> WrapPersistent(T* value, const PersistentLocation& loc = PersistentLocation()) {
  return Persistent<T>(value, loc);
}

template <typename T>
WeakPersistent<T> WrapWeakPersistent(T* value, const PersistentLocation& loc = PersistentLocation()) {
...
template <typename T, typename = std::enable_if_t<cppgc::internal::IsGarbageCollectedOrMixinType<T>::value>>
