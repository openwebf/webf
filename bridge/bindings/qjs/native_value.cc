...
NativeValue Native_NewNull() {
  return (NativeValue){0, 0, NativeTag::TAG_NULL};
}
...
JSValue nativeValueToJSValue(ExecutionContext* context, NativeValue& value) {
  switch (value.tag) {
    case NativeTag::TAG_STRING: {
      auto* string = reinterpret_cast<NativeString*>(value.u.uint64);
      if (string == nullptr)
        return JS_NULL;
    ...
    case NativeTag::TAG_INT: {
      return JS_NewUint32(context->ctx(), value.int64);
    }
    case NativeTag::TAG_BOOL: {
      return JS_NewBool(context->ctx(), value.int64 == 1);
    }
    case NativeTag::TAG_FLOAT64: {
      return JS_NewFloat64(context->ctx(), value.u.float64);
    }
    case NativeTag::TAG_NULL: {
      return JS_NULL;
    }
    case NativeTag::TAG_JSON: {
      auto* str = reinterpret_cast<const char*>(value.u.uint64);
      JSValue returnedValue = JS_ParseJSON(context->ctx(), str, strlen(str), "");
      delete str;
      return returnedValue;
    }
    case NativeTag::TAG_LIST: {
      size_t len = value.int64;
      auto* ptr = reinterpret_cast<NativeValue**>(value.u.uint64);
      JSValue array = JS_NewArray(context->ctx());
      for (int i = 0; i < len; i++) {
        NativeValue* native_value = ptr[i];
        JSValue newValue = nativeValueToJSValue(context, *native_value);
        arrayPushValue(context->ctx(), array, newValue);
        JS_FreeValue(context->ctx(), newValue);
      }
      delete ptr;
      return array;
    }
    case NativeTag::TAG_POINTER: {
      auto* ptr = reinterpret_cast<void*>(value.u.uint64);
      int64_t ptrType = value.int64;
      ...
      } else if (ptrType == static_cast<int64_t>(JSPointerType::NativeBindingObject)) {
    ...
    case NativeTag::TAG_FUNCTION: {
      int64_t functionId = value.int64;
