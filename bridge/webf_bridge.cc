...
void* initDartIsolateContext(uint64_t* dart_methods, int32_t dart_methods_len) {
  ...
  WEBF_LOG(VERBOSE) << "INIT DART ISOLATE CONTEXT: dart_context: " << dart_context_
                    << " dart_isolate_context: " << dart_isolate_context.get() << " current thread "
...
void* allocateNewPage(void* dart_isolate_context, int32_t targetContextId) {
  ...
  void* ptr = page.get();
  WEBF_LOG(VERBOSE) << "ADD NEW PAGE: dart_isolate_context: " << dart_isolate_context << " Page: " << page.get()
...
void disposePage(void* dart_isolate_context, void* page_) {
  ...
  WEBF_LOG(VERBOSE) << "DISPOSE PAGE: dart_isolate_context: " << dart_isolate_context << " Page: " << page
                    << " current thread " << std::this_thread::get_id();
...
static void finalize_dart_context(void* isolate_callback_data, void* peer) {
  ...
  WEBF_LOG(VERBOSE) << "DISPOSE DART_ISOLATE_CONTEXT:  dart_context: " << dart_context
                    << " dart_isolate_context: " << dart_isolate_context;
