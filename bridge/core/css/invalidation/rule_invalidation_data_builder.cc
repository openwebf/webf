...
void RuleInvalidationDataBuilder::Merge(const RuleInvalidationData& other) {
  for (const auto& entry : other.class_invalidation_sets) {
    MergeInvalidationSet(rule_invalidation_data_.class_invalidation_sets, entry.first, entry.second);
  }
  ...
  for (const auto& entry : other.attribute_invalidation_sets) {
    MergeInvalidationSet(rule_invalidation_data_.attribute_invalidation_sets, entry.first, entry.second);
  }
  for (const auto& entry : other.id_invalidation_sets) {
    MergeInvalidationSet(rule_invalidation_data_.id_invalidation_sets, entry.first, entry.second);
  }
  for (const auto& entry : other.pseudo_invalidation_sets) {
    auto key = static_cast<CSSSelector::PseudoType>(entry.first);
    MergeInvalidationSet(rule_invalidation_data_.pseudo_invalidation_sets, key, entry.second);
  }
...
void RuleInvalidationDataBuilder::MergeInvalidationSet(RuleInvalidationData::PseudoTypeInvalidationSetMap& map,
