...
//  const std::string& value = attribute_item.Value();
//  switch (match) {
...
//      bool has_html_space = std::find_if(selector_value.begin(), selector_value.end(), [](const char c) {
//        return IsHTMLSpace<char>(c);
//      }) != selector_value.end();
//      // Ignore empty selectors or selectors containing HTML spaces
//      if (selector_value.empty() || has_html_space) {
//        return false;
//      }
//
//      unsigned start_search_at = 0;
//      while (true) {
//        size_t found_pos = value.find(selector_value, start_search_at, case_sensitivity);
...
//        std::optional<Document::TopLayerReason> top_layer_reason =
//            html_element->GetDocument().IsScheduledForTopLayerRemoval(html_element);
