...
SelectorChecker::MatchStatus SelectorChecker::MatchForSubSelector(const SelectorCheckingContext& context,
 ...
 next_context.has_scrollbar_pseudo =
     ...
     (scrollbar_ || dynamic_pseudo == kPseudoIdScrollbarCorner || dynamic_pseudo == kPseudoIdResizer);
 ...
 if (context.in_rightmost_compound && dynamic_pseudo != kPseudoIdNone && context.pseudo_id == kPseudoIdNone) {
  ...
  case CSSSelector::kIndirectAdjacent:
   ...
   case CSSSelector::kUAShadow: {
     ...
     if (context.scope && context.scope->OwnerShadowHost() &&
         &context.scope->OwnerShadowHost()->GetTreeScope() == &context.element->GetTreeScope()) {
       return kSelectorFailsCompletely;
     }
...
//        TreeScope& host_tree_scope = next_context.selector->IsHostPseudoClass()
...
static bool AttributeValueMatches(const Attribute& attribute_item,
 ...
 switch (match) {
   ...
   case CSSSelector::kAttributeList: {
     // Check for HTML spaces in selector
     for (size_t i = 0; i < selector_value.length(); ++i) {
       if (IsHTMLSpace<char16_t>(selector_value[i])) {
         return false;  // Selectors with spaces are invalid
       }
     }
     ...
     while (start < value.length()) {
       // Skip leading spaces
       while (start < value.length() && IsHTMLSpace<char16_t>(value[start])) {
         ++start;
       }
       ...
       // Find end of token
       size_t end = start;
       while (end < value.length() && !IsHTMLSpace<char16_t>(value[end])) {
         ++end;
       }
       
       // Check if this token matches
       bool matches = false;
       if (end - start == selector_value.length()) {
         matches = true;
         for (size_t i = 0; i < selector_value.length(); ++i) {
           if ((case_sensitivity == kTextCaseSensitive && value[start + i] != selector_value[i]) ||
               (case_sensitivity != kTextCaseSensitive && 
                ToASCIILower(value[start + i]) != ToASCIILower(selector_value[i]))) {
             matches = false;
             break;
           }
         }
   ...
   case CSSSelector::kAttributeContain:
     ...
     if (case_sensitivity == kTextCaseSensitive) {
       // Simple contains check for case sensitive
       if (value.length() < selector_value.length()) {
         return false;
       }
       for (size_t i = 0; i <= value.length() - selector_value.length(); ++i) {
         bool matches = true;
         for (size_t j = 0; j < selector_value.length(); ++j) {
           if (value[i + j] != selector_value[j]) {
             matches = false;
             break;
           }
         }
       ...
       // Case-insensitive search - manually check
       if (value.length() < selector_value.length()) {
         return false;
       }
       for (size_t i = 0; i <= value.length() - selector_value.length(); ++i) {
         bool matches = true;
         for (size_t j = 0; j < selector_value.length(); ++j) {
           if (ToASCIILower(value[i + j]) != ToASCIILower(selector_value[j])) {
             matches = false;
             break;
           }
         }
   ...
   case CSSSelector::kAttributeBegin:
     ...
     if (case_sensitivity == kTextCaseSensitive) {
       ...
       for (size_t i = 0; i < selector_value.length(); ++i) {
         if (ToASCIILower(value[i]) != ToASCIILower(selector_value[i])) {
           return false;
         }
       }
       return true;
   ...
   case CSSSelector::kAttributeEnd:
     ...
     if (case_sensitivity == kTextCaseSensitive) {
       // Manual EndsWith implementation
       if (value.length() < selector_value.length()) {
         return false;
       }
       for (size_t i = 0; i < selector_value.length(); ++i) {
         if (value[value.length() - selector_value.length() + i] != selector_value[i]) {
           return false;
         }
       }
       return true;
       ...
       for (size_t i = 0; i < selector_value.length(); ++i) {
         if (ToASCIILower(value[value.length() - selector_value.length() + i]) != 
             ToASCIILower(selector_value[i])) {
           return false;
         }
       }
       return true;
   ...
   case CSSSelector::kAttributeHyphen:
     ...
     if (case_sensitivity == kTextCaseSensitive) {
       ...
       // Case-insensitive StartsWith for hyphen match
       if (value.length() < selector_value.length()) {
         return false;
       }
       for (size_t i = 0; i < selector_value.length(); ++i) {
...
inline bool CacheMatchedElementsAndReturnMatchedResultForIndirectRelation(
   ...
   Vector<Member<Element>>& has_argument_leftmost_compound_matches,
...
void SetAffectedByHasForArgumentMatchedElement(CheckPseudoHasArgumentContext& argument_context,
 ...
 for (; !affected_by_has_iterator.AtEnd(); ++affected_by_has_iterator) {
   SetAffectedByHasFlagsForElementAtDepth(argument_context, affected_by_has_iterator.CurrentElement(),
...
void AddElementIdentifierHashesInTraversalScopeAndSetAffectedByHasFlags(
 ...
 for (CheckPseudoHasArgumentTraversalIterator iterator(has_anchor_element, argument_context); !iterator.AtEnd();
      ++iterator) {
...
context,
 ...
 if (previous_result & kCheckPseudoHasResultChecked) {
   ...
   return previous_result & kCheckPseudoHasResultMatched ? kBreakEarlyAndReturnAsMatched
                                                         : kBreakEarlyAndMoveToNextArgument;
 }
 // Check fast reject filter to reject :has() argument checking early.
 bool is_new_entry;
 CheckPseudoHasFastRejectFilter& fast_reject_filter =
     cache_scope_context.EnsureFastRejectFilter(has_anchor_element, is_new_entry);
...
bool SelectorChecker::CheckPseudoHas(const SelectorCheckingContext& context, MatchResult& result) const {
 ...
 for (const CSSSelector* selector = context.selector->SelectorList()->First(); selector;
   ...
   EarlyBreakOnHasArgumentChecking early_break = CheckEarlyBreakForHasArgument(
       context, has_anchor_element, argument_context, cache_scope_context, update_affected_by_has_flags);
   ...
   if (update_affected_by_has_flags) {
     SetAffectedByHasForArgumentMatchedElement(argument_context, has_anchor_element, last_argument_checked_element,
                                               last_argument_checked_depth);
   }
   return true;
 }
...
bool SelectorChecker::CheckPseudoClass(const SelectorCheckingContext& context, MatchResult& result) const {
 ...
 switch (selector.GetPseudoType()) {
   ...
   case CSSSelector::kPseudoEmpty: {
     ...
     if (mode_ == kResolvingStyle) {
       // TODO: element.SetStyleAffectedByEmpty();
     }
     return is_empty;
   }
   ...
   case CSSSelector::kPseudoOnlyChild: {
     // TODO: Implement ViewTransition support
   ...
   case CSSSelector::kPseudoPlaceholderShown:
     // TODO: Implement text control support
  ...
  case CSSSelector::kPseudoNthLastOfType: {
   ...
   case CSSSelector::kPseudoFullPageMedia:
     // TODO: Implement IsMediaDocument
     return false;
   case CSSSelector::kPseudoDefault:
     return element.MatchesDefaultPseudoClass();
   case CSSSelector::kPseudoDisabled:
     // TODO: Implement HTMLFieldSetElement
   ...
   case CSSSelector::kPseudoUserInvalid:
     // TODO: Implement HTMLFormControlElementWithState
   ...
   case CSSSelector::kPseudoLang: {
     // TODO: Implement VTTElement support
   ...
   case CSSSelector::kPseudoDir: {
     ...
     // them now.
     // TODO: Implement slot assignment and directionality
   ...
   case CSSSelector::kPseudoDialogInTopLayer:
     // TODO: Implement HTMLDialogElement and top layer support
   ...
   case CSSSelector::kPseudoPopoverInTopLayer:
     // TODO: Implement popover support
     return false;
   case CSSSelector::kPseudoPopoverOpen:
     // TODO: Implement popover support
     return false;
   case CSSSelector::kPseudoOpen:
     // TODO: Implement open state for various elements
     return false;
   case CSSSelector::kPseudoClosed:
     // TODO: Implement closed state for various elements
     return false;
   ...
   case CSSSelector::kPseudoPaused: {
     // TODO: Implement HTMLMediaElement support
   ...
   case CSSSelector::kPseudoPermissionGranted: {
     // TODO: Implement HTMLPermissionElement support
   ...
   case CSSSelector::kPseudoPictureInPicture:
     // TODO: Implement PictureInPictureController
   ...
   case CSSSelector::kPseudoVideoPersistent: {
     DCHECK(is_ua_rule_);
     // TODO: Implement HTMLVideoElement support
   ...
   case CSSSelector::kPseudoHostContext:
     // TODO: Implement UseCounter and WebFeature
     // UseCounter::Count(context.element->GetDocument(), mode_ == kQueryingRules
   ...
   case CSSSelector::kPseudoSpatialNavigationFocus:
     DCHECK(is_ua_rule_);
     // TODO: Implement MatchesSpatialNavigationFocusPseudoClass
     // return MatchesSpatialNavigationFocusPseudoClass(element);
     return false;
   case CSSSelector::kPseudoHasDatalist:
     DCHECK(is_ua_rule_);
     // TODO: Implement MatchesHasDatalistPseudoClass
     // return MatchesHasDatalistPseudoClass(element);
     return false;
   case CSSSelector::kPseudoIsHtml:
     DCHECK(is_ua_rule_);
     // TODO: Implement HTMLDocument type check
     // return IsA<HTMLDocument>(element.GetDocument());
     return false;
   case CSSSelector::kPseudoListBox:
     DCHECK(is_ua_rule_);
     // TODO: Implement MatchesListBoxPseudoClass
     // return MatchesListBoxPseudoClass(element);
     return false;
   case CSSSelector::kPseudoMultiSelectFocus:
     DCHECK(is_ua_rule_);
     // TODO: Implement MatchesMultiSelectFocusPseudoClass
   ...
   case CSSSelector::kPseudoHostHasNonAutoAppearance:
     ...
     if (ShadowRoot* root = element.ContainingShadowRoot()) {
       ...
       // TODO: Implement ComputedStyle support
   ...
   case CSSSelector::kPseudoStateDeprecatedSyntax: {
     // TODO: Implement RuntimeEnabledFeatures
   ...
   case CSSSelector::kPseudoModal:
     ...
     //   return true;
     // }
     // TODO: Implement HTMLDialogElement support
...
//    // See https://github.com/w3c/csswg-drafts/issues/7912.
 ...
 switch (selector.GetPseudoType()) {
   ...
   case CSSSelector::kPseudoSelectFallbackButton:
     return MatchesUAShadowElement(element, shadow_element_names::kSelectFallbackButton);
   case CSSSelector::kPseudoSelectFallbackButtonIcon:
     return MatchesUAShadowElement(element, shadow_element_names::kSelectFallbackButtonIcon);
   case CSSSelector::kPseudoSelectFallbackButtonText:
     return MatchesUAShadowElement(element, shadow_element_names::kSelectFallbackButtonText);
   case CSSSelector::kPseudoSelectFallbackDatalist:
     return MatchesUAShadowElement(element, shadow_element_names::kSelectFallbackDatalist);
 ...
 if (!outer_activations.vector.empty()) {
   ...
   if (&element != ActivationCeiling(outer_activations.vector.front())) {
     if (Element* parent = element.ParentOrShadowHostElement()) {
       ...
       parent_activations =
           CalculateActivations(*parent, style_scope, outer_activations, parent_frame, parent_match_visited);
     }
   }
   ...
   for (const StyleScopeActivation& outer_activation : outer_activations.vector) {
     if (style_scope.From() ? MatchesWithScope(element, *style_scope.From(), outer_activation.root, match_visited,
