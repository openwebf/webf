...
void ProcessRule(RuleSet& rule_set,
    ...
    if (auto* layer_statement = DynamicTo<StyleRuleLayerStatement>(base_rule.get())) {
      for (const auto& name : layer_statement->GetNames()) {
        ...
        WEBF_LAZY_STREAM(WEBF_LOG_STREAM(VERBOSE), WEBF_LOG_CASCADE_IF)
            << "RuleSet(" << &rule_set << ") @layer statement ensure "
            << StyleRuleBase::LayerNameAsString(name).ToUTF8String();
      }
    }
    return;
  }
  ...
  if (base_rule->IsLayerBlockRule()) {
    if (auto* layer_block = DynamicTo<StyleRuleLayerBlock>(base_rule.get())) {
      ...
      WEBF_LAZY_STREAM(WEBF_LOG_STREAM(VERBOSE), WEBF_LOG_CASCADE_IF)
          << "RuleSet(" << &rule_set << ") @layer block enter "
          << StyleRuleBase::LayerNameAsString(layer_block->GetName()).ToUTF8String() << " => " << sub_layer;
