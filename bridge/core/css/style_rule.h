...
class StyleRuleBase {
 public:
  ...
  static AtomicString LayerNameAsString(const LayerName&);
  ...
  CSSRule* CreateCSSOMWrapper(
      uint32_t position_hint = std::numeric_limits<uint32_t>::max(),
  ...
  class ChildRuleVector {
   public:
    ...
    class Iterator {
     ...
     public:
      Iterator(const std::shared_ptr<StyleRuleBase>* position,
               const std::shared_ptr<StyleRuleBase>* end)
          : position_(position), end_(end) {
        assert(position <= end);
      }

      void operator++();
      std::shared_ptr<StyleRuleBase> operator*() const { return *position_; }
     ...
     private:
      const std::shared_ptr<StyleRuleBase>* position_;
      const std::shared_ptr<StyleRuleBase>* end_;
    };
    ...
    const std::shared_ptr<StyleRuleBase>& operator[](uint32_t i) const {
      return rules_.at(AdjustedIndex(i));
    }
    std::shared_ptr<StyleRuleBase>& operator[](uint32_t i) {
      return rules_.at(AdjustedIndex(i));
    }
    ...
    const std::vector<std::shared_ptr<StyleRuleBase>>& RawChildRules() const {
      return rules_;
    }
   ...
   private:
    ...
    uint32_t AdjustedIndex(uint32_t index) const;

    std::vector<std::shared_ptr<StyleRuleBase>> rules_;
...
//        AdditionalBytesForSelectors(selectors.size()),
        webf::PassKey<StyleRule>(), selectors, properties);
  }
  static std::shared_ptr<StyleRule> Create(std::span<CSSSelector> selectors,
...
//        AdditionalBytesForSelectors(selectors.size()),
  ...
  StyleRule(webf::PassKey<StyleRule>,
            std::span<CSSSelector> selector_vector,
            std::shared_ptr<CSSLazyPropertyParser>);
  ...
  CSSSelector& MutableSelectorAt(uint32_t index) {
    return SelectorArray()[index];
  }
  ...
  AtomicString SelectorsText() const {
...
//        AdditionalBytesForSelectors(flattened_size),
 ...
 private:
  ...
  CSSSelector* SelectorArray() {
    return reinterpret_cast<CSSSelector*>(webf::AlignUp(
...
class StyleRuleFontFace : public StyleRuleBase {
 public:
  ...
  std::shared_ptr<StyleRuleFontFace> Copy() const {
    return std::make_shared<StyleRuleFontFace>(*this);
  }
