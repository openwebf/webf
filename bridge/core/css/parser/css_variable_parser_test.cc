...
TEST_P(ValidVariableReferenceTest, ContainsValidVariableReferences) {
  ...
  auto context = std::make_shared<CSSParserContext>(
      kHTMLStandardMode);
  EXPECT_TRUE(CSSVariableParser::ContainsValidVariableReferences(
      range, context->GetExecutingContext()));
}

TEST_P(ValidVariableReferenceTest, ParseUniversalSyntaxValue) {
  SCOPED_TRACE(GetParam());
  std::shared_ptr<const CSSParserContext> context = std::make_shared<CSSParserContext>(
      kHTMLStandardMode);
  EXPECT_NE(nullptr,
