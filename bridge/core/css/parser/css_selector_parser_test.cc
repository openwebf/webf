...
//                               "::part(ident)::first-line", "::part(ident)::first-letter",
//                               "::part(ident)::selection"};
//
//   std::vector<CSSSelector> arena;
//   for (std::string test_case : test_cases) {
//     SCOPED_TRACE(test_case);
...
//   EXPECT_EQ(CSSSelector::kPseudoParent, GetImplicitlyAddedPseudo(document, ".foo:is(.bar)",
//   CSSNestingType::kNesting)); EXPECT_EQ(CSSSelector::kPseudoParent, GetImplicitlyAddedPseudo(document, "> .foo",
//   CSSNestingType::kNesting)); EXPECT_EQ(std::nullopt, GetImplicitlyAddedPseudo(document, ".foo > &",
//   CSSNestingType::kNesting)); EXPECT_EQ(std::nullopt, GetImplicitlyAddedPseudo(document, ".foo > :is(.b, &)",
//   CSSNestingType::kNesting)); EXPECT_EQ(std::nullopt, GetImplicitlyAddedPseudo(document, "& .foo",
...
//                                                                 CSSNestingType nesting_type) {
//   return CountSimpleSelectors(document, selector_text, nesting_type, [](const CSSSelector& selector) {
//     return selector.GetPseudoType() == CSSSelector::kPseudoTrue && selector.Relation() ==
//     CSSSelector::kScopeActivation;
//   });
// }
//
...
//   EXPECT_EQ(
//       4u, CountSimpleSelectors(document, ":hover > :is(:hover, .a, :hover) > :hover", CSSNestingType::kNone,
//       is_hover));
// }
//
...
//   EXPECT_EQ(0u, CountPseudoTrueWithScopeActivation(document, param.selector_text,
//   CSSNestingType::kNone).value_or(0));
// }
