...
AtomicString ConsumeStringOrURI(CSSParserTokenStream& stream, ExecutingContext* executingContext) {
  ...
  if (token.GetType() != kFunctionToken ||
      !EqualIgnoringASCIICase(token.Value(), StringView("url"))) {
    return AtomicString();
  }
...
ParseSheetResult CSSParserImpl::ParseStyleSheet(
  ...
  if (defer_property_parsing == CSSDeferPropertyParsing::kYes) {
    parser.lazy_state_ = std::make_shared<CSSLazyParsingState>(
  ...
  bool first_rule_valid = parser.ConsumeRuleList(
      ...
      [&style_sheet, &result, &string, allow_import_rules, context](
          std::shared_ptr<StyleRuleBase> rule, uint32_t offset) {
...
std::shared_ptr<StyleRuleBase> CSSParserImpl::ConsumeAtRule(
    CSSParserTokenStream& stream,
    AllowedRulesType allowed_rules,
    CSSNestingType nesting_type,
    std::shared_ptr<StyleRule> parent_rule_for_nesting) {
...
std::shared_ptr<StyleRule> CSSParserImpl::ConsumeStyleRule(
    CSSParserTokenStream& stream,
    CSSNestingType nesting_type,
    std::shared_ptr<StyleRule> parent_rule_for_nesting,
  ...
  std::span<CSSSelector> selector_vector = CSSSelectorParser::ConsumeSelector(
  ...
  if (is_css_lazy_parsing_fast_path_enabled_) {
    ...
    if (!observer_ && lazy_state_) {
      ...
      return StyleRule::Create(selector_vector,
                               std::make_shared<CSSLazyPropertyParserImpl>(
                                   block_start_offset, lazy_state_));
    }
    return ConsumeStyleRuleContents(selector_vector, stream);
  }
...
std::shared_ptr<StyleRule> CSSParserImpl::ConsumeStyleRuleContents(
