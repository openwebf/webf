...
TEST_F(CSSParserComprehensiveTest, ParseBasicProperties) {
  ...
  for (const auto& test : test_cases) {
    ...
    // For shorthands, check the expanded longhand property
    auto* value_ptr = props.GetPropertyCSSValue(test.check_property);
    ASSERT_NE(value_ptr, nullptr) << "Failed for CSS: " << test.css 
      << " property: " << static_cast<int>(test.check_property);
    ASSERT_NE(*value_ptr, nullptr) << "Failed for CSS: " << test.css;
    
    if (test.expect_identifier) {
      EXPECT_TRUE((*value_ptr)->IsIdentifierValue()) << "Failed for CSS: " << test.css;
    }
    if (test.expect_numeric) {
      EXPECT_TRUE((*value_ptr)->IsPrimitiveValue() || (*value_ptr)->IsValueList()) 
    ...
    if (test.property != test.check_property) {
      ...
      EXPECT_FALSE(shorthand_value.empty()) << "Shorthand should have value for: " << test.css;
    }
  }
}

// Test shorthand property expansion
TEST_F(CSSParserComprehensiveTest, ParseShorthandProperties) {
  struct TestCase {
    const char* css;
  ...
  TestCase test_cases[] = {
    ...
    {"p { margin: 10px; }", CSSPropertyID::kMarginTop, true},
    {"p { margin: 10px; }", CSSPropertyID::kMarginRight, true},
    {"p { margin: 10px; }", CSSPropertyID::kMarginBottom, true},
    {"p { margin: 10px; }", CSSPropertyID::kMarginLeft, true},
    
    // Margin shorthand - vertical/horizontal
    {"p { margin: 10px 20px; }", CSSPropertyID::kMarginTop, true},
    {"p { margin: 10px 20px; }", CSSPropertyID::kMarginRight, true},
    
    // Padding shorthand
    {"p { padding: 5px; }", CSSPropertyID::kPaddingTop, true},
    {"p { padding: 5px; }", CSSPropertyID::kPaddingRight, true},
    {"p { padding: 5px; }", CSSPropertyID::kPaddingBottom, true},
    {"p { padding: 5px; }", CSSPropertyID::kPaddingLeft, true},
    
    // Border shorthand
    {"p { border: 1px solid red; }", CSSPropertyID::kBorderTopWidth, true},
    {"p { border: 1px solid red; }", CSSPropertyID::kBorderTopStyle, true},
    {"p { border: 1px solid red; }", CSSPropertyID::kBorderTopColor, true},
  };
  ...
  for (const auto& test : test_cases) {
    ...
    const CSSPropertyValueSet& props = rule->Properties();
    auto* value_ptr = props.GetPropertyCSSValue(test.longhand);
    
    if (test.should_exist) {
      EXPECT_NE(value_ptr, nullptr) << "Failed for CSS: " << test.css 
        << " property: " << static_cast<int>(test.longhand);
    }
  }
}
