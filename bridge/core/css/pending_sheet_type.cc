...
std::pair<PendingSheetType, RenderBlockingBehavior> ComputePendingSheetTypeAndRenderBlockingBehavior(
  ...
  if (is_created_by_parser) {
    ...
    return std::make_pair(PendingSheetType::kBlocking, is_in_body ? RenderBlockingBehavior::kInBodyParserBlocking
                                                                  : RenderBlockingBehavior::kBlocking);
  }
  bool potentially_render_blocking = IsA<HTMLElement>(sheet_owner);
  ...
  return potentially_render_blocking
             ? std::make_pair(PendingSheetType::kDynamicRenderBlocking, RenderBlockingBehavior::kBlocking)
