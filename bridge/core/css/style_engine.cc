...
const std::vector<Member<CSSStyleSheet>>& StyleEngine::AuthorStyleSheetsInDocumentOrder() {
  if (!author_style_sheets_in_document_order_dirty_) {
    return author_style_sheets_in_document_order_;
  }

  author_style_sheets_in_document_order_.clear();

  // Walk the DOM once and cache the resulting list; this preserves cascade
  // semantics even when stylesheets finish loading out of order.
  std::vector<CSSStyleSheet*> sheets = GetDocument().styleSheets();
  author_style_sheets_in_document_order_.reserve(sheets.size());
  for (CSSStyleSheet* sheet : sheets) {
    if (!sheet) {
      continue;
    }
    author_style_sheets_in_document_order_.push_back(sheet);
  }

  author_style_sheets_in_document_order_dirty_ = false;
  return author_style_sheets_in_document_order_;
}
...
void StyleEngine::RecalcStyleForSubtree(Element& root_element) {
  Document& document = GetDocument();
  ExecutingContext* ctx = document.GetExecutingContext();
  if (!ctx || !ctx->isBlinkEnabled()) {
    return;
  }
  ...
  auto apply_for_element = [&](Element* element) -> bool {
    ...
    if (property_set && !property_set->IsEmpty()) {
      if (const auto* display_ptr = property_set->GetPropertyCSSValue(CSSPropertyID::kDisplay);
          display_ptr && *display_ptr && (*display_ptr)->IsIdentifierValue()) {
        const auto& ident = To<CSSIdentifierValue>(*(*display_ptr));
        display_none_for_invalidation = ident.GetValueID() == CSSValueID::kNone;
      } else {
    ...
    auto pseudo_bit = [](PseudoId pseudo_id) -> uint32_t {
    ...
    uint32_t sent_pseudos = element->SentPseudoStyleMask();

    auto clear_pseudo_if_sent = [&](PseudoId pseudo_id, const char* pseudo_name) {
      uint32_t bit = pseudo_bit(pseudo_id);
      if (!bit || (sent_pseudos & bit) == 0) {
        return;
      }
      ...
      command_buffer->AddCommand(UICommand::kClearPseudoStyle, pseudo_atom.ToNativeString(), element->bindingObject(),
                                 nullptr);
      sent_pseudos &= ~bit;
      element->SetSentPseudoStyleMask(sent_pseudos);
    };

    auto should_resolve_pseudo = [&](PseudoId pseudo_id) -> bool {
      uint32_t bit = pseudo_bit(pseudo_id);
      if (!bit) {
        return false;
      }
      switch (pseudo_id) {
        ...
        case kPseudoIdAfter:
          return (matched_pseudo_content_mask & bit) != 0;
        case kPseudoIdFirstLetter:
        case kPseudoIdFirstLine:
          return (matched_pseudo_mask & bit) != 0;
        default:
          return false;
      }
    };
    ...
    if (!property_set || property_set->IsEmpty()) {
      ...
      auto emit_pseudo_if_any = [&](PseudoId pseudo_id, const char* pseudo_name) {
        ...
        for (unsigned i = 0; i < pseudo_set->PropertyCount(); ++i) {
          ...
          auto* payload = reinterpret_cast<NativePseudoStyleWithHref*>(dart_malloc(sizeof(NativePseudoStyleWithHref)));
          payload->key = key_ns.release();
          payload->value = stringToNativeString(value_string).release();
          if (!base_href_string.IsEmpty()) {
            payload->href = stringToNativeString(base_href_string).release();
          } else {
            payload->href = nullptr;
          }
          command_buffer->AddCommand(UICommand::kSetPseudoStyle, pseudo_atom.ToNativeString(), element->bindingObject(),
                                     payload);
        }
        uint32_t bit = pseudo_bit(pseudo_id);
        if (bit) {
      ...
      if (emit_before) {
        ...
        clear_pseudo_if_sent(PseudoId::kPseudoIdBefore, "before");
      }
      if (emit_after) {
        (void)emit_pseudo_if_any(PseudoId::kPseudoIdAfter, "after");
      } else {
        clear_pseudo_if_sent(PseudoId::kPseudoIdAfter, "after");
      }
      if (emit_first_letter) {
        (void)emit_pseudo_if_any(PseudoId::kPseudoIdFirstLetter, "first-letter");
      } else {
        clear_pseudo_if_sent(PseudoId::kPseudoIdFirstLetter, "first-letter");
      }
      if (emit_first_line) {
    ...
    for (unsigned i = 0; i < count; ++i) {
      ...
      command_buffer->AddStyleByIdCommand(element->bindingObject(), static_cast<int32_t>(id), value_slot, base_href,
                                               prop.IsImportant());
    }
...
void StyleEngine::RecalcStyleForElementOnly(Element& element) {
  ...
  auto apply_for_element = [&](Element* el) {
    ...
    uint32_t sent_pseudos = el->SentPseudoStyleMask();
    ...
    auto clear_pseudo_if_sent = [&](PseudoId pseudo_id, const char* pseudo_name) {
      ...
      command_buffer->AddCommand(UICommand::kClearPseudoStyle, pseudo_atom.ToNativeString(), el->bindingObject(),
                                 nullptr);
      sent_pseudos &= ~bit;
      el->SetSentPseudoStyleMask(sent_pseudos);
    };
    ...
    if (!property_set || property_set->IsEmpty()) {
      ...
      auto emit_pseudo_if_any = [&](PseudoId pseudo_id, const char* pseudo_name) {
        ...
        for (unsigned i = 0; i < pseudo_set->PropertyCount(); ++i) {
          ...
          command_buffer->AddCommand(UICommand::kSetPseudoStyle, pseudo_atom.ToNativeString(), el->bindingObject(),
                                     payload);
        }
...
void StyleEngine::Trace(GCVisitor* visitor) {
  ...
  for (const auto& author_style_sheets_in_document_order : author_style_sheets_in_document_order_) {
    visitor->TraceMember(author_style_sheets_in_document_order);
  }
