...
#ifndef WEBF_RENDERER_CORE_CSS_WHITE_SPACE_H_
#define WEBF_RENDERER_CORE_CSS_WHITE_SPACE_H_
...
//
constexpr uint8_t ToWhiteSpaceValue(WhiteSpaceCollapse collapse, TextWrap wrap) {
...
enum class EWhiteSpace : uint8_t {
  ...
  kPreLine = ToWhiteSpaceValue(WhiteSpaceCollapse::kPreserveBreaks, TextWrap::kWrap),
  kPreWrap = ToWhiteSpaceValue(WhiteSpaceCollapse::kPreserve, TextWrap::kWrap),
  kBreakSpaces = ToWhiteSpaceValue(WhiteSpaceCollapse::kBreakSpaces, TextWrap::kWrap),
};

static_assert(kWhiteSpaceCollapseBits + kTextWrapBits <= sizeof(EWhiteSpace) * 8);
...
inline bool IsValidWhiteSpace(EWhiteSpace whitespace) {
  return whitespace == EWhiteSpace::kNormal || whitespace == EWhiteSpace::kNowrap || whitespace == EWhiteSpace::kPre ||
         whitespace == EWhiteSpace::kPreLine || whitespace == EWhiteSpace::kPreWrap ||
