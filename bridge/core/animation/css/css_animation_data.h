...
class CSSAnimationData final : public CSSTimingData {
 public:
  ...
  std::unique_ptr<CSSAnimationData> Clone() const { return base::WrapUnique(new CSSAnimationData(*this)); }

  bool AnimationsMatchForStyleRecalc(const CSSAnimationData& other) const;
  bool operator==(const CSSAnimationData& other) const { return AnimationsMatchForStyleRecalc(other); }
  ...
  const std::vector<double>& IterationCountList() const { return iteration_count_list_; }
  const std::vector<Timing::PlaybackDirection>& DirectionList() const { return direction_list_; }
  const std::vector<Timing::FillMode>& FillModeList() const { return fill_mode_list_; }
  const std::vector<EAnimPlayState>& PlayStateList() const { return play_state_list_; }
  const std::vector<std::optional<TimelineOffset>>& RangeStartList() const { return range_start_list_; }
  const std::vector<std::optional<TimelineOffset>>& RangeEndList() const { return range_end_list_; }

  const std::vector<EffectModel::CompositeOperation>& CompositionList() const { return composition_list_; }
  ...
  std::vector<std::optional<TimelineOffset>>& RangeStartList() { return range_start_list_; }
  std::vector<std::optional<TimelineOffset>>& RangeEndList() { return range_end_list_; }
  std::vector<EffectModel::CompositeOperation>& CompositionList() { return composition_list_; }
  ...
  static std::optional<TimelineOffset> InitialRangeEnd() { return std::nullopt; }
