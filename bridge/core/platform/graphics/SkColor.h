...
*/
[[nodiscard]] static constexpr inline SkColor SkColorSetA(SkColor c, unsigned a) {
...
*/
void SkRGBToHSV(unsigned red, unsigned green, unsigned blue, float hsv[3]);
...
*/
SkColor SkHSVToColor(unsigned alpha, const float hsv[3]);
...
*/
SkPMColor SkPreMultiplyARGB(unsigned a, unsigned r, unsigned g, unsigned b);
...
struct SkRGBA4f {
  ...
  */
  float operator[](int index) const {
    static_cast<void>(index >= 0 && index < 4);
  ...
  */
  bool isOpaque() const {
    static_cast<void>(fA <= 1.0f && fA >= 0.0f);
    return fA == 1.0f;
  }
