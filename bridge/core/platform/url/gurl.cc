...
std::string_view GURL::PathForRequestPiece() const {
  assert_m(parsed_.path.is_nonempty(), "Canonical path for requests should be non-empty");
...
const GURL& GURL::EmptyGURL() {
  static webf::NoDestructor<GURL> empty_gurl;
  return *empty_gurl;
}
...
size_t GURL::EstimateMemoryUsage() const {
  return parsed_.inner_parsed() ? sizeof(url::Parsed) : 0;
...
bool GURL::IsAboutPath(std::string_view actual_path,
  ...
  if (!actual_path.starts_with(allowed_path))
    return false;

  if (actual_path.size() == allowed_path.size()) {
    assert(actual_path == allowed_path);
    return true;
  }

  if ((actual_path.size() == allowed_path.size() + 1) &&
      actual_path.back() == '/') {
    assert(actual_path == std::string(allowed_path) + '/');
    return true;
  }
...
bool operator==(const GURL& x, std::string_view spec) {
  assert_m(GURL(spec).possibly_invalid_spec() == spec, "Comparisons of GURLs and strings must ensure as a precondition that the string is fully canonicalized.");
