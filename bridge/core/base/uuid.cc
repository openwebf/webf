...
Uuid Uuid::GenerateRandomV4() {
  ...
  std::span<const uint8_t, 16> input = RandBytes(sixteen_bytes);
...
Uuid Uuid::FormatRandomDataAsV4(
    ...
    webf::PassKey<content::FileSystemAccessManagerImpl> /*pass_key*/) {
  return FormatRandomDataAsV4Impl(input);
}

// static
Uuid Uuid::FormatRandomDataAsV4ForTesting(std::span<const uint8_t, 16> input) {
  return FormatRandomDataAsV4Impl(input);
}

// static
Uuid Uuid::FormatRandomDataAsV4Impl(std::span<const uint8_t, 16> input) {
  assert(input.size_bytes() == kGuidV4InputLength);
  ...
  Uuid uuid;
  uuid.lowercase_ = std::format("{:08x}-{:04x}-{:04x}-{:04x}-{:012x}",
