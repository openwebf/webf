...
double RandDouble() {
  return BitsToOpenEndedUnitInterval(webf::RandUint64());
}

float RandFloat() {
  return BitsToOpenEndedUnitIntervalF(webf::RandUint64());
}
/*
...
uint64_t RandGenerator(uint64_t range) {
  ...
  uint64_t value;
  do {
    value = webf::RandUint64();
  } while (value > max_acceptable_value);

  return value % range;
}
/* // TODO(guopengfei)：先注释，用不到
...
MetricsSubSampler::ScopedNeverSampleForTesting::~ScopedNeverSampleForTesting() {
  assert(!g_subsampling_always_sample);
  assert(g_subsampling_never_sample);
