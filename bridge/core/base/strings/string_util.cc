...
#include "core/base/strings/string_util_impl_helpers.h"
#include "foundation/macros.h"

namespace base {

// CountUnicodeCharacters ------------------------------------------------------
inline bool IsValidCodepoint(int32_t code_point) {
...
inline bool IsValidCharacter(int32_t code_point) {
...
const std::string& EmptyString() {
  static const webf::NoDestructor<std::string> s;
  return *s;
}

const std::u16string& EmptyString16() {
  static const webf::NoDestructor<std::u16string> s16;
  return *s16;
}
...
bool EqualsASCII(std::u16string_view str, std::string_view ascii) {
  return webf::ranges::equal(ascii, str);
}
...
std::u16string FormatBytesUnlocalized(int64_t bytes) {
  ...
  return  std::u16string(std::string_view(buf).begin(), std::string_view(buf).end());;
}
...
std::u16string JoinString(std::span<const std::u16string> parts,
                          std::u16string_view separator) {
  return internal::JoinStringT(parts, separator);
}

std::string JoinString(std::span<const std::string_view> parts,
                       std::string_view separator) {
  return internal::JoinStringT(parts, separator);
}

std::u16string JoinString(std::span<const std::u16string_view> parts,
                          std::u16string_view separator) {
  return internal::JoinStringT(parts, separator);
}
...
uint32_t Find(const int8_t* characters,
              uint32_t length,
...
uint32_t ReverseFind(const CharacterType* characters,
                            uint32_t length,
...
bool ContainsOnlyASCIIOrEmpty(const std::string& str) {
  return std::empty(str) || std::all_of(str.begin(), str.end(), [](unsigned char c) {
    return isascii(c);
  });
}
