...
class CheckedNumeric {
 ...
 public:
  ...
  template <typename Src, typename = std::enable_if_t<std::is_arithmetic_v<Src>>>
  // NOLINTNEXTLINE(google-explicit-constructor)
  ...
  template <typename Src>
  // NOLINTNEXTLINE(google-explicit-constructor)
  constexpr CheckedNumeric(StrictNumeric<Src> value) : state_(static_cast<Src>(value)) {}
  ...
  constexpr CheckedNumeric operator-() const {
    ...
    const bool is_valid = IsValid() && (!std::is_signed_v<T> || std::is_floating_point_v<T> ||
...
L* operator+(L* lhs, const StrictNumeric<R> rhs) {
  const uintptr_t result = CheckAdd(reinterpret_cast<uintptr_t>(lhs), CheckMul(sizeof(L), static_cast<R>(rhs)))
...
template <typename L, typename R>
L* operator-(L* lhs, const StrictNumeric<R> rhs) {
  const uintptr_t result = CheckSub(reinterpret_cast<uintptr_t>(lhs), CheckMul(sizeof(L), static_cast<R>(rhs)))
                               .template ValueOrDie<uintptr_t>();
  return reinterpret_cast<L*>(result);
}
