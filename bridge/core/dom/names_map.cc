...
void NamesMap::Add(const AtomicString& key, const AtomicString& value) {
  // AddResult
  auto [add_result, is_new_entry] = data_.insert(std::make_pair(key, nullptr));
  if (is_new_entry) {
    add_result->second = std::make_shared<SpaceSplitString>();
  }
  add_result->second->Add(value);
}
...
void NamesMap::Set(const AtomicString& source, const CharacterType* characters) {
  Clear();
