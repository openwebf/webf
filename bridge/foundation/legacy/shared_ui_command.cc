...
LegacySharedUICommand::LegacySharedUICommand(ExecutingContext* context)
    : context_(context),
      ...
      ui_command_sync_strategy_(std::make_unique<LegacyUICommandSyncStrategy>(this)),
      is_blocking_reading_(false),
      is_blocking_writing_(false) {}

void LegacySharedUICommand::AddCommand(UICommand type,
...
void LegacySharedUICommand::SyncToReserve() {
...
void LegacySharedUICommand::ConfigureSyncCommandBufferSize(size_t size) {
  ui_command_sync_strategy_->ConfigWaitingBufferSize(size);
}

void LegacySharedUICommand::SyncToActive() {
  SyncToReserve();
...
void LegacySharedUICommand::appendCommand(std::unique_ptr<UICommandBuffer>& target,
