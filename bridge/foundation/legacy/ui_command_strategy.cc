...
LegacyUICommandSyncStrategy::LegacyUICommandSyncStrategy(LegacySharedUICommand* host) : host_(host) {}

bool LegacyUICommandSyncStrategy::ShouldSync() {
  return should_sync;
}

void LegacyUICommandSyncStrategy::Reset() {
  should_sync = false;
  waiting_status.Reset();
  frequency_map_.clear();
}
void LegacyUICommandSyncStrategy::RecordUICommand(UICommand type,
...
void LegacyUICommandSyncStrategy::ConfigWaitingBufferSize(size_t size) {
...
void LegacyUICommandSyncStrategy::SyncToReserve() {
  host_->SyncToReserve();
  waiting_status.Reset();
  frequency_map_.clear();
  should_sync = true;
...
void LegacyUICommandSyncStrategy::SyncToReserveIfNecessary() {
  if (frequency_map_.size() > waiting_status.MaxSize() && waiting_status.IsFullActive()) {
    SyncToReserve();
  }
}

void LegacyUICommandSyncStrategy::RecordOperationForPointer(NativeBindingObject* ptr) {
  size_t index;
