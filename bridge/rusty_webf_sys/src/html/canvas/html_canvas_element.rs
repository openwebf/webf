...
pub struct HTMLCanvasElement {
  ...
  method_pointer: *const HTMLCanvasElementRustMethods,
}
impl HTMLCanvasElement {
  pub fn initialize(ptr: *const OpaquePtr, context: *const ExecutingContext, method_pointer: *const HTMLCanvasElementRustMethods, status: *const RustValueStatus) -> HTMLCanvasElement {
    unsafe {
      HTMLCanvasElement {
  ...
  pub fn get_context(&self, context_type: &str, exception_state: &ExceptionState) -> Result<CanvasRenderingContext, String> {
    let value = unsafe {
      ((*self.method_pointer).get_context)(self.ptr(), CString::new(context_type).unwrap().as_ptr(), exception_state.ptr)
    };
    if exception_state.has_exception() {
      return Err(exception_state.stringify(self.context()));
    }
    Ok(CanvasRenderingContext::initialize(value.value, self.context(), value.method_pointer, value.status))
  }
}
...
impl HTMLCanvasElementMethods for HTMLCanvasElement {
  ...
  fn get_context(&self, context_type: &str, exception_state: &ExceptionState) -> Result<CanvasRenderingContext, String> {
    self.get_context(context_type, exception_state)
  }
  fn as_html_canvas_element(&self) -> &HTMLCanvasElement {
    self
  }
...
impl NodeMethods for HTMLCanvasElement {
...
impl EventTargetMethods for HTMLCanvasElement {
