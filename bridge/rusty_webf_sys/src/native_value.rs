...
impl NativeValue {
  ...
  pub fn new_list(values: Vec<NativeValue>) -> Self {
    ...
    for (i, val) in values.iter().enumerate() {
      ...
      if value.is_string() {
        let inner_str = value.to_string();
        let new_string_ptr = Self::create_string_ptr(&inner_str, inner_str.len());
        value.u.ptr = new_string_ptr as *mut c_void;
      }
      unsafe {
...
impl Drop for NativeValue {
  fn drop(&mut self) {
    if self.is_string() {
      ...
      let shared_native_string = unsafe {
        ptr.read()
      };
