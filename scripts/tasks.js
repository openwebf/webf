...
task('build-window-webf-lib', (done) => {
  ...
  // generate project
  execSync(`cmake -DCMAKE_BUILD_TYPE=${buildType} -DAPP_VERSION="${appVersion}" ${isProfile ? '-DENABLE_PROFILE=TRUE' : ''} ${'-DENABLE_TEST=true '} -G "Ninja" -B ${paths.bridge}\\cmake-build-windows -S ${paths.bridge}`,
    {
      cwd: paths.bridge,
      stdio: 'inherit',
      env: {
