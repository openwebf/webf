...
// Placeholder task - type generation now handled in merge-webf-and-polyfill-typings
task('generate-typings', (done) => {
  console.log(chalk.yellow('Type generation is now handled in merge-webf-and-polyfill-typings task'));
  done();
});

// Extract polyfill types directly from source and merge with webf.d.ts
task('merge-webf-and-polyfill-typings', (done) => {
  try {
    console.log(chalk.blue('Extracting polyfill types and merging with webf.d.ts...'));
    
    const typingsPath = join(WEBF_ROOT, 'bridge/typings');
    const webfDtsPath = join(typingsPath, 'webf.d.ts');
    const polyfillSrcPath = join(WEBF_ROOT, 'bridge/polyfill/src');
    
    // Check if webf.d.ts exists
    if (!fs.existsSync(webfDtsPath)) {
      throw new Error('webf.d.ts not found');
    }
    
    // Read webf.d.ts
    const webfContent = readFileSync(webfDtsPath, 'utf-8');
    
    // Extract polyfill type definitions from source files
    console.log(chalk.yellow('Extracting polyfill types from source files...'));
    
    // Use TypeScript compiler API to extract types
    const ts = require('typescript');
    const glob = require('glob');
    const path = require('path');
    
    // Find all TypeScript files in polyfill/src
    const polyfillFiles = glob.sync('**/*.ts', {
      cwd: polyfillSrcPath,
      absolute: true,
      ignore: ['**/test/**', '**/*.test.ts', '**/*.spec.ts', '**/index.ts']
    });
    
    // Add the main index.ts file last to ensure dependencies are resolved
    const indexPath = path.join(polyfillSrcPath, 'index.ts');
    if (fs.existsSync(indexPath)) {
      polyfillFiles.push(indexPath);
    }
    
    // Create a temporary directory for generated declarations
    const tempDir = path.join(WEBF_ROOT, 'bridge/polyfill/.temp-types');
    if (!fs.existsSync(tempDir)) {
      fs.mkdirSync(tempDir, { recursive: true });
    }
    
    // Use TypeScript compiler to generate declaration files
    const program = ts.createProgram(polyfillFiles, {
      declaration: true,
    ...
    // Read generated declaration files
    let polyfillTypes = '';
    const generatedFiles = glob.sync('**/*.d.ts', {
      cwd: tempDir,
      absolute: true
    });
    
    // Process generated declaration files
    for (const dtsFile of generatedFiles) {
      const content = readFileSync(dtsFile, 'utf-8');
      
      // Extract exported interfaces, types, and classes
      ...
      for (const line of lines) {
        ...
        if (line.startsWith('import') || line.startsWith('export * from') || line.trim() === '') {
          continue;
        }
        ...
        if (line.startsWith('declare ')) {
          if (currentDeclaration) {
            // Special handling for Webf interface to ensure proper type references
            if (currentDeclaration.includes('interface Webf')) {
              // The generated types might not include the proper Document and Window references
              // so we keep the declaration as-is
            }
            polyfillTypes += currentDeclaration + '\n\n';
          }
        ...
        // Handle export declarations
          ...
          currentDeclaration = line.replace(/^export /, '');
          inExport = true;
          braceCount = (line.match(/{/g) || []).length - (line.match(/}/g) || []).length;
        } else if (inExport) {
          currentDeclaration += '\n' + line;
          ...
          if (braceCount === 0 && line.includes('}')) {
            // Special handling for Webf interface
            if (currentDeclaration.includes('interface Webf')) {
              // Ensure Document and Window types are preserved
              currentDeclaration = currentDeclaration
                .replace(/document: any/, 'document: Document')
                .replace(/window: any/, 'window: Window & typeof globalThis');
            }
            polyfillTypes += currentDeclaration + '\n\n';
            currentDeclaration = '';
            inExport = false;
    ...
    // Add manually defined polyfill interfaces that are internal
    const manualPolyfillTypes = `
// Storage interfaces
...
type BodyInit = string | Blob | ArrayBuffer | FormData | URLSearchParams | null;
...
// Other polyfill types
type MatchMedia = (mediaQuery: string) => MediaQueryList;
...
`;
    ...
    // Merge polyfill types with webf.d.ts
    let mergedContent = webfContent;
    
    // Insert polyfill types before the webf namespace declaration
    const namespaceIndex = mergedContent.indexOf('declare namespace webf');
    if (namespaceIndex !== -1) {
      mergedContent = mergedContent.slice(0, namespaceIndex) +
        '\n// ===== Polyfill Type Definitions =====\n' +
        polyfillTypes +
        manualPolyfillTypes +
        '\n' +
        mergedContent.slice(namespaceIndex);
    } else {
      // If no namespace found, append at the end
      mergedContent += '\n// ===== Polyfill Type Definitions =====\n' + polyfillTypes + manualPolyfillTypes;
    }
    
    // Ensure Webf is added to the webf namespace
    const namespaceMatch = mergedContent.match(/declare namespace webf \{([\s\S]*?)\}/m);
    if (namespaceMatch && !namespaceMatch[1].includes('export type Webf')) {
      // Add Webf to the namespace
      const namespaceContent = namespaceMatch[1].trimEnd() + '\n  export type Webf = globalThis.Webf;\n';
      mergedContent = mergedContent.replace(namespaceMatch[0], `declare namespace webf {${namespaceContent}}`);
    }
    
    // Write the merged content back to webf.d.ts
    writeFileSync(webfDtsPath, mergedContent);
    
    console.log(chalk.green('Successfully extracted and merged polyfill types with webf.d.ts'));
    done();
  } catch (err) {
    console.error(chalk.red('Error extracting polyfill types:'), err);
    done(err);
  }
});

// Update index.d.ts to create namespace wrapper for WebF types
task('merge-all-typings', (done) => {
  try {
    console.log(chalk.blue('Creating namespace wrapper for WebF types...'));
    ...
    // Read webf.d.ts to extract type names
    const webfContent = readFileSync(webfDtsPath, 'utf-8');
    
    // Extract all interface and type names from webf.d.ts to declare them globally
    const interfaceRegex = /^(?:export\s+)?interface\s+(\w+)/gm;
    const typeRegex = /^(?:export\s+)?(?:type|declare\s+class)\s+(\w+)/gm;
    const interfaces = [];
    const types = [];
    let match;
    
    // Extract interfaces
    while ((match = interfaceRegex.exec(webfContent)) !== null) {
      const interfaceName = match[1];
      // Skip internal type aliases and helper types
      if (!interfaceName.includes('Init') &&
          interfaceName !== 'IDLEventHandler' &&
          interfaceName !== 'EventListenerOptions' &&
          interfaceName !== 'AddEventListenerOptions') {
        interfaces.push(interfaceName);
      }
    }
    
    // Extract types
    while ((match = typeRegex.exec(webfContent)) !== null) {
      const typeName = match[1];
      // Skip internal type aliases and helper types
      if (!typeName.startsWith('Dart') && 
          !typeName.startsWith('Static') && 
          !typeName.startsWith('Support') && 
          !typeName.startsWith('Dependents') &&
          !typeName.includes('Init') &&
          typeName !== 'IDLEventHandler' &&
          typeName !== 'EventListener' &&
          typeName !== 'LegacyNullToEmptyString' &&
          typeName !== 'BlobPart' &&
          // Don't redeclare types that are interfaces
          !interfaces.includes(typeName)) {
        types.push(typeName);
      }
    }
    
    // Update index.d.ts to properly export webf types under the webf namespace only
...
${interfaces.map(interfaceName => `    export interface ${interfaceName} extends WebFTypes.${interfaceName} {}`).join('\n')}
    
    // Re-export all types
${types.map(typeName => `    export type ${typeName} = WebFTypes.${typeName};`).join('\n')}
  }
}
...
`;
    ...
    writeFileSync(indexDtsPath, updatedIndexContent);
    
    console.log(chalk.green('Successfully updated index.d.ts with WebF namespace wrapper'));
    done();
  } catch (err) {
    console.error(chalk.red('Error merging all type files:'), err);
    done(err);
  }
});
