...
class FlutterSliverListview extends WidgetElement {
  ...
  static StaticDefinedSyncBindingObjectMethodMap listViewMethods = {
    'finishRefresh': StaticDefinedSyncBindingObjectMethod(
        call: (bindingObject, args) => castToType<FlutterSliverListview>(bindingObject)
            .finishRefresh(args.isNotEmpty ? args[0] : 'success')),
    'finishLoad': StaticDefinedSyncBindingObjectMethod(
        call: (bindingObject, args) => castToType<FlutterSliverListview>(bindingObject)
            .finishLoad(args.isNotEmpty ? args[0] : 'success')),
    'resetHeader': StaticDefinedSyncBindingObjectMethod(
        call: (bindingObject, args) =>
            castToType<FlutterSliverListview>(bindingObject).resetHeader()),
    'resetFooter': StaticDefinedSyncBindingObjectMethod(
        call: (bindingObject, args) =>
            castToType<FlutterSliverListview>(bindingObject).resetFooter())
  };
...
class FlutterSliverListViewState extends WebFWidgetElementState {
  ...
  @override
  FlutterSliverListview get widgetElement => super.widgetElement as FlutterSliverListview;
  ...
  void _scrollListener() {
    if (!mounted ||
        ...
        (scrollController?.positions.isEmpty ?? false)) {
      return;
    }
  ...
  Widget buildListViewItemByIndex(int index) {
    ...
    if (node is dom.Element) {
      ...
      return LayoutBoxWrapper(
          ownerElement: node, child: widgetElement.childNodes.elementAt(index).toWidget());
    }
