  ...
  try {
    ...
    if (QuickJSByteCodeCacheObject.cacheMode == ByteCodeCacheMode.DEFAULT &&
      ...
      if (!result) {
        ...
        entry?.end(metadata: {'cached': true, 'fallback': true});
        return evaluateScripts(contextId, codeBytes, scriptElement: scriptElement);
      }
      entry?.end(metadata: {'cached': true});
      return result;
    } else {
      ...
      try {
        ...
        entry?.end(metadata: {'cached': false});
        return result;
      } catch (e, stack) {
        bridgeLogger.severe('Error in event listener', e, stack);
        entry?.end(metadata: {'error': e.toString()});
      }
...
Future<void> parseHTML(double contextId, Uint8List codeBytes) async {
  ...
  final entry = PerformanceTracker.instance.beginEntry(
      kSubTypeHtmlParse, 'parseHTML', metadata: {'size': codeBytes.length});
  Pointer<Uint8> codePtr = uint8ListToPointer(codeBytes);
  try {
    assert(_allocatedPages.containsKey(contextId));
...
Future<Uint8List> dumpQuickjsByteCode(double contextId, Uint8List code,
  ...
  _dumpQuickjsByteCode(_allocatedPages[contextId]!, codePtr, code.length, bytecodes,
      bytecodeLen, urlPtr, isModule, context, resultCallback);
...
typedef NativeAllocateNewPage = Void Function(Double, Int32, Int8, Pointer<Void>, Pointer<WidgetElementShape>, Int32 shapeLen,
    Handle object, Pointer<NativeFunction<HandleAllocateNewPageResult>> handleResult);
typedef DartAllocateNewPage = void Function(double, int, int, Pointer<Void>, Pointer<WidgetElementShape>, int shapeLen,
    Object object, Pointer<NativeFunction<HandleAllocateNewPageResult>> handleResult);
