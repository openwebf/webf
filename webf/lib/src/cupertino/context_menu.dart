...
class FlutterCupertinoContextMenu extends WidgetElement {
    ...
    'setActions': StaticDefinedSyncBindingObjectMethod(
      call: (element, args) {
        ...
        if (args.isNotEmpty) {
            ...
            for (var item in actionsData) {
              if (item is Map) {
                ...
                newActions.add(Map<String, dynamic>.from(item.map((key, value) => MapEntry(key.toString(), value))));
                 ...
                 print('Skipping non-map item in actions list: $item');
              }
            }
          ...
          menu.setState(() {}); // Update the UI after actions change
        } else {
  ...
  List<Widget> _buildActions() {
    ...
    for (final action in _actions) {
      ...
      builtActions.add(
        CupertinoContextMenuAction(
          ...
          onPressed: () {
            dispatchEvent(CustomEvent(eventName)); // Dispatch event with the name from config
            ...
            try {
              ...
              print('Error closing menu: $e\n$stacktrace');
            }
          },
          trailingIcon: iconName != null ? _getIconData(iconName) : null,
          child: Text(text),
        )
      );
    }
    ...
    if (builtActions.isEmpty) {
       print("Warning: _buildActions called but resulted in empty list. This shouldn't happen if build logic is correct.");
       // Return a dummy action to prevent crashes, though ideally build method prevents this call.
       return [ CupertinoContextMenuAction(child: const Text('Error'), onPressed: () => Navigator.of(context!, rootNavigator: true).pop())];
    }
  ...
  IconData? _getIconData(String iconName) {
    switch (iconName) {
      case 'share': return CupertinoIcons.share;
      case 'heart': return CupertinoIcons.heart;
      case 'delete': return CupertinoIcons.delete;
      case 'doc': return CupertinoIcons.doc;
      case 'doc_text': return CupertinoIcons.doc_text;
      case 'phone': return CupertinoIcons.phone;
      case 'chat_bubble': return CupertinoIcons.chat_bubble;
      case 'mail': return CupertinoIcons.mail;
      case 'photo': return CupertinoIcons.photo;
      case 'add': return CupertinoIcons.add;
      case 'edit': return CupertinoIcons.pencil;
      case 'pencil': return CupertinoIcons.pencil;
      case 'person_circle': return CupertinoIcons.person_circle;
      case 'folder': return CupertinoIcons.folder;
      case 'star': return CupertinoIcons.star;
      case 'xmark_circle': return CupertinoIcons.xmark_circle;
      default: 
      ...
      if (child is dom.Element) { 
        ...
        if (childWidget != null) {
          ...
          return Container(
           ...
           print('Warning: First element child ${child.tagName} failed to render to a widget.');
        }
