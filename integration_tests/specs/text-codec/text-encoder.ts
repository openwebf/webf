describe('TextEncoder', () => {
  ...
  describe('encodeInto method', () => {
    ...
    it('should handle emoji characters at buffer boundary', () => {
      ...
      expect(result.read).toBe(3); // 2 UTF-16 code units for hi + 2 for surrogate pair
    ...
    it('should throw TypeError for invalid destination', () => {
      const encoder = new TextEncoder();
      expect(() => encoder.encodeInto('test', null as any)).toThrow(TypeError);
      expect(() => encoder.encodeInto('test', undefined as any)).toThrow(TypeError);
      expect(() => encoder.encodeInto('test', [] as any)).toThrow(TypeError);
    });
