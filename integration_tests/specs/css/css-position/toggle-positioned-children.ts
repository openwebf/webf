describe('toggle-positioned-children', () => {
  it('should maintain icon position when toggling container display', async (done) => {
    const style = document.createElement('style');
    style.textContent = `
      .container {
        display: block;
        position: relative;
        left: 50%;
        top: 50%;
        right: 50%;
        width: 300px;
        margin-left: -150px;
        border: 1px solid #000;
      }

      .wrapper {
        display: flex;
        padding: 10px;
        justify-content: center;
        flex-direction: column;
      }

      .top {
        padding: 25px;
        padding-left: 45px;
        font-size: 20px;
        font-weight: 700;
        display: flex;
        justify-content: center;
        flex-direction: column;
      }

      .imgText {
        border: 1px solid #000;
        flex-direction: column;
      }

      .imgText img {
        width: 20px;
        height: 20px;
        margin-right: 5px;
        position: absolute;
        top: 30px;
        left: 20px;
      }
    `;
    document.head.appendChild(style);

    const container = createElement('div', {
      className: 'container'
    }, [
      createElement('div', {
        className: 'wrapper'
      }, [
        createElement('div', {
          className: 'top'
        }, [
          createElement('div', {
            className: 'imgText'
          }, [
            createElement('img', {
              src: 'data:image/png;base64,'
            }),
            createElement('span', {}, [
              createText('Header')
            ]),
            createElement('div', {}, [
              createText('The positioned icon should remain in same position when toggle.')
            ])
          ])
        ])
      ])
    ]);

    document.body.appendChild(container);

    requestAnimationFrame(async () => {
      await snapshot(1.0);
      
      await snapshot();
      
      container.style.display = 'none';
      container.style.display = 'block';
      
      await snapshot();
      
      done();
    });
  });
});