...
.three {border-top-width: 25px;}`)
    ...
    const p2 = createElement('p', {
      ...
      createText(', which should result in a thin solid border on the top side of each box in the inline element (and the UA\'s default border on the other three sides).  There is also an '),
      createElement('span', {
        className: 'three'
      }, [
        createText('inline element of class '),
      ...
      createText(', which should have no top border width or visible border because no border style was set.')
    ]);
