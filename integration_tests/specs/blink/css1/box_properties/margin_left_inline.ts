describe('CSS1 margin-left inline', () => {
  xit('margin-left property on inline elements', async () => {
    const p1 = createElementWithStyle('p', {
      ...
      createText(', which should result in 25-pixel left margin only in the '),
    ...
    const p2 = createElementWithStyle('p', {
      ...
      createText(', which should result in -10px left margin only in the '),
      createElement('strong', {}, [createText('first')]),
      createText(' line in which the inline box appears.')
    ]);
