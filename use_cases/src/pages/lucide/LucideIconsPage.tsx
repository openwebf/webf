...
export const LucideIconsPage: React.FC = () => {
  ...
  const filteredIcons = debouncedSearchTerm
    ? allIconNames.filter(name => name.toLowerCase().includes(debouncedSearchTerm.toLowerCase()))
    : [];
                  ...
                  <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2 max-h-96 overflow-y-auto">
                    {filteredIcons.slice(0, 50).map((iconName) => (
                      ...
                      >
                        ...
                        <span
                          className="text-xs text-fg-secondary text-center leading-tight break-all line-clamp-2 w-full flex items-center justify-center">{iconName}</span>
                      </div>
                    ))}
                  </div>
                ) : (
